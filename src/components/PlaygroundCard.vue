  ...
  <div class="container" v-bind:style="{ backgroundImage: 'url(' + image + ')' }">
    <div class="container__profile">
      ...
      <div class="container__profile__text">
        <h2>{{name}}</h2>
        <button @click="modalStateOpen = true">Details</button>
      </div>
    </div>
  </div>
    <teleport to="body">
...
export default {
  ...
  data() {
    return {
      ...
      latitude: this.playground.location.latitude,
      longitude: this.playground.location.longitude,
      city: this.playground.location.city,
      streetAddress: this.playground.location.street,
      zip: this.playground.location.zip,
      image: this.playground.heroImage,
